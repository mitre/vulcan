...
| `DATABASE_PORT` | PostgreSQL client connection port (used by database.yml) | `5432` | `5436` |
...
| `POSTGRES_PORT` | Docker host-side port mapping (should match DATABASE_PORT) | `5432` | `5436` |
| `POSTGRES_USER` | PostgreSQL username (Docker init + database.yml) | `postgres` | `vulcan` |
| `POSTGRES_PASSWORD` | PostgreSQL password (Docker init + database.yml) | - | `secure_password` |
| `POSTGRES_DB` | PostgreSQL database name (Docker init + production database.yml) | `vulcan_postgres_production` | `vulcan_db` |
...
| `RAILS_FORCE_SSL` | Force SSL connections (set to `false` for local Docker testing without SSL) | `true` | `false` |
