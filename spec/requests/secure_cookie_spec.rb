...
# Regression test: Secure cookie flag must NOT be set explicitly in session config
...
RSpec.describe 'Secure cookie configuration', type: :request do
  before do
  ...
  describe 'session store' do
    it 'does not explicitly set the secure flag (Rails handles this via force_ssl)' do
      ...
      expect(session_options).not_to have_key(:secure),
        'Session config must NOT set `secure:` explicitly. ' \
