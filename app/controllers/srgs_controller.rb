...
class SrgsController < ApplicationController
  ...
  def parse_control(group, cci_items)
    control = {
      control_params: {},
      nist_params: {}
    }
    ...
    control[:control_params][:description]   = group.rule.description.gsub(%r{<\w?*>|<\/\w?*>}, '')
    ...
    control[:nist_params] = parse_nist_params(cci_items.fetch_nists(group.rule.idents))
    control
  end
