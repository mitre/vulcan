...
export default {
  ...
  computed: {
    ...
    filteredSelectRulesForCheckbox() {
...
<template>
  ...
  <div class="row">
    <div class="col-12">
      <div class="row">
        <h2>
          <i v-if="rule.locked" class="bi bi-lock" aria-hidden="true" />
          <i v-if="rule.review_requestor_id" class="bi bi-file-earmark-search" aria-hidden="true" />
          <i v-if="rule.changes_requested" class="bi bi-exclamation-triangle" aria-hidden="true" />
          <a
            class="headerLink"
      ...
      <div v-if="!readOnly">
        ...
        <p v-else>
          Created on {{ friendlyDateTime(rule.created_at) }}
        </p>
        ...
        <NewRuleModalForm
          ...
          @rule-selected="$emit('ruleSelected', $event.id)"
        />
        <b-button v-b-modal.duplicate-rule-modal variant="info">
          Clone Control
        </b-button>
          ...
          <div class="reviewDropdownCard">
            <b-card v-if="showReviewPane" class="shadow">
              ...
              <template #header>
                ...
                <i class="bi bi-x" aria-hidden="true" @click="showReviewPane = false" />
              </template>
              ...
              <b-form-group label="" class="mb-0">
                ...
                >
                  <p class="mb-0">
                    <small><strong>{{ action.name }}</strong></small>
                  </p>
                  <small><em>{{ action.description }}</em></small>
                </b-form-radio>
              </b-form-group>
