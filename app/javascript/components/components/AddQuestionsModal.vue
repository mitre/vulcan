...
export default {
  ...
  methods: {
    ...
    removeQuestions(index) {
      const currentQuestion = this.questions.splice(index, 1)[0]
    ...
    updateAdditionalQuestionsSuccess(response) {
        ...
        <div v-for="(data, index) in questions" :key="index" class="pb-2">
          <b-input-group>
            ...
            <b-col sm="1" class="px-0">
              <b-button variant="danger" class="ml-2" @click="removeQuestions(index)">
                X
              </b-button>
            </b-col>
          </b-input-group>
