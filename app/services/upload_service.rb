...
class UploadService
  ...
  def upload_project_inspec_tarball(file, current_user)
    Open3.capture3("mv #{file.path} #{file.path.split('.')[0]}.tar.gz")
    stdout_json, = Open3.capture3("inspec json #{file.path.split('.')[0]}.tar.gz")
    json = stdout_json.split("\n", 2)[1] == '' ? stdout_json : stdout_json.split("\n", 2)[1]
    upload_project_inspec_json(JSON.parse(json), current_user)
  end
  ...
  def upload_project_url(url, current_user)
    stdout, = Open3.capture3("inspec json #{url}")
    json = stdout.split("\n", 2)[1]
    upload_project_inspec_json(JSON.parse(json), current_user)
  end
  ...
  def upload_project_excel(file, current_user, opts)
    File.open('tmp/mapping.yml', 'w') { |opfile| opfile.write(opts.to_yaml) }
    ...
    upload_project_inspec_json(inspec_json, current_user)
  end
  ...
  def create_project_controls(project, inspec_json)
  ...
  def upload_project_inspec_json(inspec_json, current_user)
    ...
    @project.save
    create_project_controls(@project, inspec_json)
    @project
  end
